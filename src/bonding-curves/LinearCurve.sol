...
contract LinearCurve is ICurve, CurveErrorCodes {
        ...
        )
    {
        ...
        uint256 buySpotPrice = priceInfoParams.spotPrice + priceInfoParams.delta;
        ...
        inputValue =
            ...
            buySpotPrice +
            (priceInfoParams.numItems * (priceInfoParams.numItems - 1) * priceInfoParams.delta) /
            2;
        ...
        )
    {
        ...
        uint256 totalPriceDecrease = priceInfoParams.delta * priceInfoParams.numItems;

        // If the current spot price is less than the total amount that the spot price should change by...
        uint256 newNumItems = priceInfoParams.numItems;
        if (priceInfoParams.spotPrice < totalPriceDecrease) {
            ...
            uint256 numItemsTillZeroPrice = priceInfoParams.spotPrice / priceInfoParams.delta + 1;
            newNumItems = numItemsTillZeroPrice;
        }
        ...
        // Thus we don't need to calculate the maximum number of items until we reach zero spot price, so we don't modify numItems
            ...
            newSpotPrice = priceInfoParams.spotPrice - uint128(totalPriceDecrease);
        }
        ...
        outputValue =
            newNumItems *
            priceInfoParams.spotPrice -
            (newNumItems * (newNumItems - 1) * priceInfoParams.delta) /
            2;
